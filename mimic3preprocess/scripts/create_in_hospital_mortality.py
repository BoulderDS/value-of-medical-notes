...
def process_partition(args, partition, eps = 1e-6):
    ...
    for (patient_index, patient) in enumerate(patients):
        ...
        for ts_filename in patient_ts_files:
            ...
            # make sure patient does not die in given period
            if n_hours <= 49 and ts_df['Hours'].dropna().iloc[-1] < n_hours:
                print("\n\t(last event is shorter than given period)", patient, ts_filename)
                continue
    ...
    with open(os.path.join(output_dir, f"{partition}_{period}_listfile.csv"), "w") as listfile:
