...
def process_partition(args, partition, eps = 1e-6):
    ...
    for (patient_index, patient) in enumerate(patients):
        ...
        for ts_filename in patient_ts_files:
            ...
            if 'CGID' not in ts_df.columns:
                ...
                continue
            is_note = 1 - pd.isnull(ts_df[Config.note_type[args.note]]).values.astype(int)
            if np.sum(is_note) == 0: # there is no notes in this admission
                continue
    ...
    period = str(int(args.partition_type)) if args.partition_type != -1 else "retro"
    with open(os.path.join(output_dir, f"{partition}_{args.note}_{period}_listfile.csv"), "w") as listfile:
...
if __name__ == '__main__':
    ...
    parser.add_argument('note', type=str, choices=['all', 'all_but_discharge', 'physician', 'discharge', 'physician_nursing'], help="Directory where the created data should be stored.")
